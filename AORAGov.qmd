...
MonitorCountOut <- dplyr::summarise(
  MonitorLog2,
  Count = n(),
  .by = DateStamp
) |> arrange(
  ...
  CumulativeCount = cumsum(Count),
  DateStampNum = as.numeric(DateStamp)
  ) 

MonitorCountIn <- dplyr::summarise(
  MonitorResponse3,
  Count = n(),
  .by = DateStamp
) |> arrange(
...
MonitorCountAll <- left_join(
  MonitorCountOut |> rename(
    CumulativeOut = "CumulativeCount"
  ),
  MonitorCountIn |> dplyr::select(
    DateStampNum,
    CumulativeCount
  ) |> rename(
    CumulativeIn = "CumulativeCount"
  ),
  by = "DateStampNum"
) |> 
  arrange(DateStamp) |> 
  # Handle missing values using lag for CumulativeIn, zero for CumulativeOut
  mutate(
    CumulativeIn = case_when(
      is.na(CumulativeIn) & row_number() == 1 ~ 0,  # First row gets 0 if missing
      is.na(CumulativeIn) ~ lag(CumulativeIn),      # Use previous value if missing
      TRUE ~ CumulativeIn                           # Keep original value
    ),
    CumulativeOut = replace_na(CumulativeOut, 0)
  ) |> 
  # Fill forward any remaining NAs in CumulativeIn
  fill(CumulativeIn, .direction = "down") |> 
  # Reshape to long format for plotting
  pivot_longer(
    cols = c(CumulativeIn, CumulativeOut),
    names_to = "Type",
    values_to = "CumulativeCount"
  ) |> 
  mutate(
    Type = str_remove(Type, "Cumulative"),
    Type = factor(Type, levels = c("Out", "In"))
  )

# Create the plot
Monitor_Cumulative <- ggplot(MonitorCountAll, aes(x = DateStamp, y = CumulativeCount, color = Type)) +
  geom_line(linewidth = 1) +
  geom_point(size = 1.5, alpha = 0.7) +
  # Add smooth curves for each type
  geom_smooth(
    method = "gam", 
    formula = y ~ s(x, bs = "cs"), 
    linewidth = 1.2, 
  ...
  # Labels and title
  labs(
    title = "Cumulative Message Counts Over Time",
    x = "Date",
    y = "Cumulative Count",
    color = "Type"
  ) +
...
# Print the plot
knitr::knit_print(Monitor_Cumulative)
```
...
# Alternative function for overlaid plot (all cohorts on same panel)
create_response_rate_overlay <- function(response_data, rate_type = "daily") {
...
# Create response rate data and plots
...
# Overlay plots (all cohorts on same panel)
ResponseRate_Daily_Overlay <- create_response_rate_overlay(ResponseRates_Daily, "daily")
ResponseRate_Weekly_Overlay <- create_response_rate_overlay(ResponseRates_Weekly, "weekly")

# Print the plots
knitr::knit_print(ResponseRate_Daily_Plot)
knitr::knit_print(ResponseRate_Weekly_Plot)
knitr::knit_print(ResponseRate_Daily_Overlay)
knitr::knit_print(ResponseRate_Weekly_Overlay)
...
Monitor_Cumulative_Faceted <- ggplot(MonitorCountAll_Long, 
  ...
  labs(
    title = "Cumulative Message Counts Over Time by Cohort",
    x = "Date",
    y = "Cumulative Count",
    color = "Type"
  ) +
...
# Create weekly alert rate plot
weekly_alert_rate <- MonitorResponse4 %>%
  # Create week variable using floor_date to group by week
  ...
  # Filter out weeks with very few responses (optional - adjust threshold as needed)
  filter(Total_Responses >= 5)

# Create the plot with trend line
ggplot(weekly_alert_rate, aes(x = Week, y = Alert_Rate)) +
  # Add raw data points and lines
  geom_line(color = "red", alpha = 0.6, size = 0.8) +
  geom_point(color = "red", alpha = 0.7, size = 1.5) +
  # Add smooth trend line using spline
  geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs"), 
              color = "darkblue", size = 1.2, se = TRUE, alpha = 0.3) +
  scale_y_continuous(
    labels = scales::percent_format(accuracy = 1),
    limits = c(0, max(weekly_alert_rate$Alert_Rate) * 1.1)
  ) +
  scale_x_date(
    date_labels = "%b %Y",
  ...
  labs(
    title = "Alert Response Rate by Week",
    subtitle = "Proportion of responses flagged as 'Alert' per week (with trend line)",
    x = "Month",
    y = "Alert Rate (%)",
    caption = paste("Total responses:", nrow(MonitorResponse4), "| Blue line shows smoothed trend")
  ) +
  theme_minimal() +
  theme(
...
# Calculate and report trend statistics
library(mgcv)

# Fit GAM model for trend analysis
gam_model <- gam(Alert_Rate ~ s(as.numeric(Week)), data = weekly_alert_rate)
trend_summary <- summary(gam_model)

# Calculate overall trend direction
first_quarter <- weekly_alert_rate %>% 
  slice_head(n = ceiling(nrow(weekly_alert_rate) * 0.25)) %>%
  summarise(avg_rate = mean(Alert_Rate))

last_quarter <- weekly_alert_rate %>% 
  slice_tail(n = ceiling(nrow(weekly_alert_rate) * 0.25)) %>%
  summarise(avg_rate = mean(Alert_Rate))

trend_direction <- if_else(last_quarter$avg_rate > first_quarter$avg_rate, "increasing", "decreasing")
trend_magnitude <- abs(last_quarter$avg_rate - first_quarter$avg_rate)

cat("TREND ANALYSIS:\n")
cat("===============\n")
cat("Overall trend direction:", trend_direction, "\n")
cat("Magnitude of change:", scales::percent(trend_magnitude, accuracy = 0.1), "\n")
cat("First quarter average:", scales::percent(first_quarter$avg_rate, accuracy = 0.1), "\n")
cat("Last quarter average:", scales::percent(last_quarter$avg_rate, accuracy = 0.1), "\n")
cat("GAM model R-squared:", round(trend_summary$r.sq, 3), "\n")
cat("GAM model p-value:", format.pval(trend_summary$s.pv, digits = 3), "\n\n")

cat("\nWeeks with highest alert rates:\n")
print(weekly_alert_rate %>% 
  arrange(desc(Alert_Rate)) %>% 
  head(5) %>%
  mutate(Alert_Rate_Pct = scales::percent(Alert_Rate, accuracy = 0.1)))

cat("\nWeeks with most total responses:\n")
print(weekly_alert_rate %>% 
